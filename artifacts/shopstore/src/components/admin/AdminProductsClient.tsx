...
const DEMO_PRODUCTS: Product[] = [
  { id: 1, name: "Smart Fitness Watch", slug: "smart-fitness-watch", price: "149.00", originalPrice: "199.00", images: ["https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=200"], stock: 10, featured: true, trending: false, category: { id: 1, name: "Electronics", slug: "electronics" } },
  { id: 2, name: "Noise Cancelling Headphones", slug: "noise-cancelling-headphones", price: "229.00", images: ["https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200"], stock: 5, featured: true, trending: true, category: { id: 1, name: "Electronics", slug: "electronics" } },
  { id: 3, name: "Premium Cotton T-Shirt", slug: "premium-cotton-tshirt", price: "39.00", images: ["https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=200"], stock: 20, featured: false, trending: false, category: { id: 2, name: "Fashion", slug: "fashion" } },
];
...
export function AdminProductsClient({ products: serverProducts, categories: serverCategories }: { products: Product[]; categories: Category[] }) {
  const [products, setProducts] = useState<Product[]>(serverProducts.length > 0 ? serverProducts : DEMO_PRODUCTS);
  const categories = serverCategories.length > 0 ? serverCategories : DEMO_CATEGORIES;
  ...
  const generateDescription = async () => {
    if (!form.name && uploadedImages.length === 0) {
      toast.error("Add a product name or image first");
      return;
    }
    ...
    try {
      const res = await fetch("/api/ai/generate-description", {
        ...
        body: JSON.stringify({ productName: form.name, imageUrl: uploadedImages[0] }),
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editProduct) {
        setProducts((p) => p.map((prod) => prod.id === editProduct.id ? { ...prod, ...form, images: uploadedImages } : prod));
        toast.success("Product updated!");
      } else {
        setProducts((p) => [...p, { id: Date.now(), ...form, images: uploadedImages, slug: form.name.toLowerCase().replace(/\s+/g, "-"), category: categories.find((c) => c.id === parseInt(form.categoryId)) ?? null }]);
        toast.success("Product added!");
      }
      setModalOpen(false);
    } catch {
      toast.error("Failed to save product — using local state");
      if (editProduct) {
        setProducts((p) => p.map((prod) => prod.id === editProduct.id ? { ...prod, ...form, images: uploadedImages } : prod));
      } else {
        setProducts((p) => [...p, { id: Date.now(), ...form, images: uploadedImages, slug: form.name.toLowerCase().replace(/\s+/g, "-"), category: categories.find((c) => c.id === parseInt(form.categoryId)) ?? null }]);
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await fetch(`/api/admin/products/${id}`, { method: "DELETE" }); } catch {}
                  ...
                  <tr key={product.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <img src={(product.images as string[])?.[0] ?? ""} alt={product.name} className="w-10 h-10 rounded-xl object-cover bg-gray-100" onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=40"; }} />
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => handleDelete(product.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <form onSubmit={handleSave} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <input type="text" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} required className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900/20" placeholder="e.g. Smart Fitness Watch" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select value={form.categoryId} onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none bg-white">
                ...
                <div className="flex gap-4">
                  {[
                    ...
                    <label key={key} className="flex items-center gap-2 cursor-pointer">
                      <button type="button" onClick={() => setForm((f) => ({ ...f, [key]: !f[key as keyof FormData] }))} className={`relative w-10 h-5 rounded-full transition-colors ${form[key as keyof FormData] ? "bg-gray-900" : "bg-gray-200"}`}>
                        <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${form[key as keyof FormData] ? "translate-x-5" : "translate-x-0.5"}`} />
                      </button>
