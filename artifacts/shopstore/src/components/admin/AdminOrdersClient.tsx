...
const DEMO_ORDERS = [
  { id: 1, orderNumber: "ORD-892471", createdAt: new Date("2025-03-20"), total: "149.00", status: "processing", user: { name: "Jane Doe", email: "jane@example.com" }, items: [{ productName: "Smart Fitness Watch", quantity: 1, price: "149.00" }] },
  { id: 2, orderNumber: "ORD-783291", createdAt: new Date("2025-03-18"), total: "458.00", status: "delivered", user: { name: "John Smith", email: "john@example.com" }, items: [{ productName: "Noise Cancelling Headphones", quantity: 2, price: "229.00" }] },
  { id: 3, orderNumber: "ORD-675432", createdAt: new Date("2025-03-15"), total: "79.00", status: "pending", user: { name: "Alice Johnson", email: "alice@example.com" }, items: [{ productName: "Wireless Game Controller", quantity: 1, price: "79.00" }] },
];
...
export function AdminOrdersClient({ orders: serverOrders }: { orders: any[] }) {
  const displayOrders = serverOrders.length > 0 ? serverOrders : DEMO_ORDERS;
  const [orders, setOrders] = useState(displayOrders);
                  ...
                  <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className={`text-xs font-semibold px-3 py-1.5 rounded-full border-0 cursor-pointer focus:outline-none ${STATUS_COLORS[order.status as keyof typeof STATUS_COLORS] ?? "bg-gray-100 text-gray-700"}`}
                      >
